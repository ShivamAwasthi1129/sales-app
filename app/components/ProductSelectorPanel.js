...
export default function ProductSelectorPanel({ 
  ...
  useEffect(() => {
    ...
    if (editingProduct && products && products.length > 0) {
      ...
      if (product) {
        console.log('[ProductSelectorPanel] Pre-populating product for editing:', product);
        ...
        console.log('[ProductSelectorPanel] Pre-populated options:', { initialOptions, initialCheckboxes });
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-2">
                {editingProduct && <span>✏️</span>}
                {showProductList ? 'Select Product' : (editingProduct ? 'Edit Product' : 'Configure Product')}
              </h3>
              <p className="text-white/70 text-xs">
                {showProductList 
                  ? (editingProduct ? 'Or choose a different product' : 'Choose from available products')
          ...
          <div className="p-4">
            ...
            <div className="mb-4">
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 pl-9 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                />
                <svg
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-gray-500 text-sm">No products available</p>
                <p className="mt-2 text-xs text-gray-400">Please create products in Catalogue first.</p>
              </div>
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-gray-500 text-sm">No products match your search</p>
                <p className="mt-2 text-xs text-gray-400">Try a different search term.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {filteredProducts.map((product) => (
                  <div
                    ...
                    className="p-3 rounded-lg border-2 border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 cursor-pointer transition-all"
                  >
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="bg-white rounded-lg p-3 border border-gray-200">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-700 font-medium">Base Price</span>
            ...
            {selectedProduct.attributes && selectedProduct.attributes.length > 0 && (
              <div className="space-y-3">
                <h4 className="font-semibold text-gray-900 text-sm">Configure Options</h4>
                {selectedProduct.attributes.map((attribute) => (
                  <div key={attribute.id} className="bg-white rounded-lg p-3 border border-gray-200">
                          ...
                          if (selectedOption && selectedOption.description) {
                            return (
                              <div className="bg-indigo-50 rounded-lg p-2 border border-indigo-200">
                                <p className="text-xs text-gray-700">{selectedOption.description}</p>
                              </div>
                            );
                          }
                          return null;
                    ...
                    {attribute.uiType === 'checkbox' && (
                      <div className="space-y-2">
                        {attribute.options.map((option) => {
                          ...
                          return (
                            <label key={option.id} className="flex items-center space-x-2 p-2 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                              <input
                                type="checkbox"
                                checked={isChecked}
                                onChange={(e) => {
                          ...
                          return (
                            <div key={option.id} className="p-3 border border-gray-200 rounded-lg">
                              <div className="flex justify-between items-center mb-1">
                                <label className="text-xs font-medium text-gray-700">{option.label}</label>
                    ...
                    {attribute.uiType === 'number_input' && attribute.options && attribute.options.length > 0 && (
                      <div className="space-y-2">
                        {attribute.options.map((option) => {
                          ...
                          return (
                            <div key={option.id} className="p-2 border border-gray-200 rounded-lg">
                              <div className="flex justify-between items-center mb-1">
                                <label className="text-xs font-medium text-gray-700">{option.label}</label>
                                <span className="text-xs font-bold text-indigo-600">
            ...
            <button
              ...
              className="w-full px-4 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-medium hover:from-indigo-700 hover:to-purple-700 transition-colors text-sm shadow-lg"
            >
