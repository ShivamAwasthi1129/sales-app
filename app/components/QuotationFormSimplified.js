// components/QuotationFormSimplified.js
...
const QuotationFormSimplified = forwardRef(
  ({ onQuotationCreated, onCancel }, ref) => {
    ...
    const [createQuotation, { loading: creatingQuotation }] =
      useMutation(CREATE_QUOTATION);
    const [updateQuotation, { loading: updatingQuotation }] =
      useMutation(UPDATE_QUOTATION);
    const [
      getQuotation,
      { data: quotationData, loading: loadingQuotation, error: quotationError },
    ] = useLazyQuery(GET_QUOTATION, {
    ...
    const [validateCoupon, { loading: validatingCouponQuery }] = useLazyQuery(
      VALIDATE_COUPON,
      {
        fetchPolicy: "network-only",
      }
    ...
    const { data: currentSalesPersonData } = useQuery(
      GET_CURRENT_USER_SALESPERSON,
      {
        fetchPolicy: "cache-and-network",
    ...
    const companyId =
      currentSalesPersonData?.getCurrentUser?.companyId ||
    ...
    useEffect(() => {
      if (
        currentSalesPersonData?.getCurrentUser &&
        currentSalesPersonData.getCurrentUser.role === "Sales Person"
      ) {
    ...
    useEffect(() => {
      if (
        ...
        const activeSalesPersons = salesPersonsData.getSalesPersons.filter(
          (sp) =>
            sp.status === "Active" && sp.companyId === currentUser.companyId
        );
        if (activeSalesPersons.length > 0 && !selectedSalesPerson) {
          ...
          setSalesPersonSearch(
            `${firstSalesPerson.name} (${firstSalesPerson.salesPersonId})`
          );
          console.log(
            "[QuotationFormSimplified] Auto-selected sales person for Admin:",
            firstSalesPerson
          );
        }
      }
    ...
    useEffect(() => {
      if (
        notesAndTermsData?.getNotesAndTerms &&
        !isEditMode &&
        !editingQuotationId
      ) {
        const companyNotes =
          notesAndTermsData.getNotesAndTerms.notesToClient || "";
        const companyTerms =
          notesAndTermsData.getNotesAndTerms.termsAndConditions || "";
    ...
    useEffect(() => {
      ...
      if (quotationData?.getQuotation && isEditMode && editingQuotationId) {
        ...
        console.log(
          "[QuotationFormSimplified] ✅ Loading quotation data for edit:",
          quotation
        );
        ...
        if (quotation.couponCode && quotation.couponDiscount > 0) {
          console.log(
            "[QuotationFormSimplified] Re-applying saved coupon:",
        ...
        if (quotation.from?.salesPersonId) {
          ...
          if (sp) {
            ...
            setSalesPersonSearch(
              `${quotation.from.salesPersonName} (${quotation.from.salesPersonId})`
            );
          }
        }
        ...
        if (quotation.to?.businessName && quotation.to?.email) {
          setClientSearch(
            `${quotation.to.businessName} (${quotation.to.email})`
          );
          ...
          if (existingClient) {
            ...
            console.log(
              "[QuotationFormSimplified] Found existing client:",
              existingClient
            );
          } else {
            ...
            console.log(
              "[QuotationFormSimplified] Client not found in customers list, treating as new client"
            );
          }
        }

        console.log(
          "[QuotationFormSimplified] ✅ Form data loaded successfully"
        );
        console.log(
          "[QuotationFormSimplified] Current isEditMode:",
          isEditMode
        );
        console.log(
          "[QuotationFormSimplified] Current editingQuotationId:",
          editingQuotationId
        );
      } else {
        console.log(
          "[QuotationFormSimplified] ⚠️ Not loading quotation data - conditions not met"
        );
      }
    }, [
      quotationData,
    ...
    useImperativeHandle(ref, () => ({
      loadQuotationForEdit: (quotationId) => {
        console.log(
          "[QuotationFormSimplified] ========== LOAD FOR EDIT =========="
        );
        ...
        console.log(
          "[QuotationFormSimplified] Setting editingQuotationId to:",
          quotationId
        );
    ...
    const handleSubmit = async (sendEmailFlag = true) => {
      try {
        console.log(
          "[QuotationFormSimplified] ========== SUBMIT START =========="
        );
        ...
        if (!formData.to.businessName?.trim()) {
          validationErrors.push(
            "Client/Business name is required. Please add client details before saving."
          );
        }
        if (!formData.to.email?.trim()) {
          validationErrors.push(
            "Client email is required. Please add client details before saving."
          );
        } else if (
          !formData.to.email.includes("@") ||
          !formData.to.email.includes(".")
        ) {
          validationErrors.push(
            "Valid client email address is required (e.g., client@example.com)"
          );
        }
        ...
        if (
          ...
          validationErrors.push(
            "Sales person information is missing. Please select a sales person."
          );
        }
        ...
        formData.lineItems.forEach((item, index) => {
          ...
          if (!item.rate || item.rate <= 0) {
            validationErrors.push(
              `Product ${index + 1}: Rate/Price must be greater than 0`
            );
          }
        });
        ...
        if (validationErrors.length > 0) {
          console.log(
            "[QuotationFormSimplified] ❌ Validation errors:",
            validationErrors
          );
          const errorMessage =
        ...
        console.log(
          "[QuotationFormSimplified] Proceeding to build quotation input..."
        );
        ...
        const clientBusinessName = selectedClient
          ...
          : newClientData.customerName.trim() ||
          formData.to.businessName.trim();
        ...
        const quotationInput = {
          ...
          couponCode:
            appliedCoupons.length > 0
              ? appliedCoupons.map((c) => c.coupon?.code).join(", ")
              : "",
        ...
        console.log(
          "[QuotationFormSimplified] quotationInput built:",
          JSON.stringify(quotationInput, null, 2)
        );
        ...
        if (isEditMode && editingQuotationId) {
          ...
          console.log(
            "[QuotationFormSimplified] Updating quotation ID:",
            editingQuotationId
          );
          console.log(
            "[QuotationFormSimplified] Send email flag:",
            sendEmailFlag
          );
          ...
          console.log(
            "[QuotationFormSimplified] Update input:",
          ...
          console.log(
            "[QuotationFormSimplified] Update response errors:",
            errors
          );

          if (errors && errors.length > 0) {
            console.error(
              "[QuotationFormSimplified] Update GraphQL Errors:",
              errors
            );
          ...
          if (data?.updateQuotation) {
            console.log(
              "[QuotationFormSimplified] ✅ Quotation updated successfully"
            );
            ...
            console.error(
              "[QuotationFormSimplified] No data returned from update mutation"
            );
          ...
          console.log(
            "[QuotationFormSimplified] ✅ ENTERING CREATE MODE (Not edit mode)"
          );
          console.log(
            "[QuotationFormSimplified] Creating quotation with sendEmail:",
            sendEmailFlag
          );
          console.log(
            "[QuotationFormSimplified] Quotation input:",
            JSON.stringify(quotationInput, null, 2)
          );
          ...
          if (data?.createQuotation) {
            console.log(
              "[QuotationFormSimplified] Quotation created successfully:",
            ...
            if (sendEmailFlag) {
              toast.success(
                `✅ Quotation Created Successfully!\n\nQuotation #${data.createQuotation.quotationNo
                  ...
                  data.createQuotation.quotationDate
                ).toLocaleDateString()}`,
                {
                  autoClose: 4000,
                  position: "top-center",
            ...
            if (sendEmailFlag) {
              ...
              console.log(
                "[QuotationFormSimplified] Switched to edit mode for draft:",
                data.createQuotation.id
              );
            }
            ...
            console.error(
              "[QuotationFormSimplified] No data returned from mutation"
            );
        ...
        console.error(
          "[QuotationFormSimplified] ❌ Error with quotation:",
          error
        );
        ...
        if (error.graphQLErrors && error.graphQLErrors.length > 0) {
          ...
          if (
            firstError.includes('Field "isSubscription" is not defined') ||
            firstError.includes('Field "billingInterval" is not defined') ||
            firstError.includes("SelectedOptionInput")
          ) {
            errorMessage =
              "❌ Product configuration error. Please refresh the page and try reconfiguring the product.";
          } else if (
            firstError.includes("businessName") ||
            firstError.includes("Client information") ||
            firstError.includes("to.businessName") ||
            firstError.includes("to.email")
          ) {
            errorMessage =
              "❌ Client information is required. Please fill in client name and email before saving.";
          } else if (firstError.includes("__typename")) {
            errorMessage =
              "❌ Data format error. Please try refreshing the page and try again.";
          } else if (firstError.includes("duplicate")) {
            errorMessage =
              "❌ This quotation already exists. Please check and try again.";
          } else if (firstError.includes("not found")) {
            errorMessage =
              "❌ Referenced item not found. Please refresh and try again.";
          } else if (
            firstError.includes("Validation failed") &&
            ...
            errorMessage =
              "❌ Required field missing. Please fill all required fields.";
          } else {
        ...
        console.log(
          "[QuotationFormSimplified] User shown error:",
          errorMessage
        );
      } finally {
        setIsSavingDraft(false);
    ...
    const handleApplyCoupon = async (autoCode = null) => {
      ...
      try {
        ...
        const groupIds = [
          ...new Set(
            formData.lineItems.map((item) => item.groupId).filter(Boolean)
          ),
        ];
        ...
        if (data?.validateCoupon) {
          ...
          if (data.validateCoupon.valid) {
            ...
            const currentDiscount = appliedCoupons.reduce(
              (sum, c) => sum + (c.discount || 0),
              0
            );
            const newTotalDiscount =
              currentDiscount + data.validateCoupon.discount;
            ...
            if (newTotalDiscount > subtotal) {
              ...
              toast.error(
                `Cannot apply coupon. Total discount ($${newTotalDiscount.toFixed(
                  2
                )}) would exceed subtotal ($${subtotal.toFixed(
                  2
                )}). Maximum allowed: $${maxAllowedDiscount.toFixed(2)}`
              );
              return;
            }
            ...
            const existingCouponIndex = appliedCoupons.findIndex(
              (c) => c.coupon.type === couponType
            );
            ...
            if (existingCouponIndex !== -1) {
              ...
              const newTotal =
                currentDiscount - oldDiscount + data.validateCoupon.discount;
              if (newTotal > subtotal) {
                toast.error(
                  `Cannot replace coupon. Total discount ($${newTotal.toFixed(
                    2
                  )}) would exceed subtotal ($${subtotal.toFixed(2)})`
                );
                return;
              }
              ...
              toast.success(
                `Coupon "${data.validateCoupon.coupon.code
                }" replaced previous ${couponType} coupon! You save $${data.validateCoupon.discount.toFixed(
                  2
                )}`
              );
            } else {
              ...
              toast.success(
                `Coupon "${data.validateCoupon.coupon.code
                }" applied! You save $${data.validateCoupon.discount.toFixed(
                  2
                )}`
              );
            }
    ...
    const handleRemoveCoupon = (couponCode) => {
      setAppliedCoupons(
        appliedCoupons.filter((c) => c.coupon.code !== couponCode)
      );
    ...
    const handleAddProduct = (product, selectedOptions = []) => {
      console.log(
        "[QuotationFormSimplified] Adding/Updating product:",
        product
      );
      console.log(
        "[QuotationFormSimplified] Selected options:",
        selectedOptions
      );
      console.log(
        "[QuotationFormSimplified] Editing index:",
      ...
      const lineItem = {
        ...
        subscriptionDetails:
          product.basePrice?.billingType === "recurring"
            ...
            : null,
        subscriptionPrice:
          product.basePrice?.billingType === "recurring" ? baseAmount : null,
      ...
      console.log(
        "[QuotationFormSimplified] Current line items before update:",
        formData.lineItems
      );
      ...
      if (
        ...
        editingLineItemIndex < formData.lineItems.length
      ) {
        // Update existing item
        console.log(
          "[QuotationFormSimplified] Updating item at index:",
          editingLineItemIndex
        );
        setFormData((prev) => {
          const updatedLineItems = prev.lineItems.map((item, idx) => {
            if (idx === editingLineItemIndex) {
              console.log(
                "[QuotationFormSimplified] Replacing item:",
                item,
                "with:",
                lineItem
              );
          ...
          console.log(
            "[QuotationFormSimplified] Updated line items:",
            updatedLineItems
          );
          return {
            ...prev,
                ...
                if (data?.validateCoupon?.valid) {
                  setAppliedCoupons((prev) => {
                    ...
                    const exists = prev.some(
                      (c) => c.coupon.id === data.validateCoupon.coupon.id
                    );
                    if (exists) return prev;
                    return [...prev, data.validateCoupon];
                  });
                  toast.success(
                    `🎉 Group discount "${matchingCoupon.code
                    }" auto-applied! Save $${data.validateCoupon.discount.toFixed(
                      2
                    )}`
                  );
                }
    ...
    useEffect(() => {
      const handleClickOutside = (event) => {
        if (
          !event.target.closest(".sales-person-dropdown") &&
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex-1 lg:max-w-[65%]">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                  ...
                  <div className="bg-white rounded-lg p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all">
                    ...
                    <div className="relative sales-person-dropdown">
                      <input
                        ...
                        className="w-full px-4 py-2.5 text-sm border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white transition-all"
                      ...
                      {showSalesPersonDropdown &&
                        salesPersonsData?.getSalesPersons && (
                          <div className="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                            ...
                            {salesPersonsData.getSalesPersons.filter(
                              (sp) =>
                                !salesPersonSearch ||
                                sp.name
                                  ?.toLowerCase()
                    ...
                    {selectedSalesPerson &&
                      selectedSalesPerson.id !== currentSalesPerson?.id && (
                ...
                {/* Client Details */}
                <div className="bg-white rounded-lg p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all">
                  <h3 className="text-lg font-bold text-blue-900 mb-4 border-b border-gray-200 pb-2">
                    Client Details
                  </h3>
                  ...
                  {currentUser?.role !== "Customer" && !selectedClient && (
                    <div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full px-4 py-2.5 text-sm border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                      ...
                      <p className="mt-3 text-xs text-gray-600">
                        Note: Customer account will be created when quotation is
                        sent
                      </p>
                    </div>
                  )}
                ...
                >
                  <ProductSelectorPanel
                    ...
                    editingProduct={
                      editingLineItemIndex !== null
                        ? formData.lineItems[editingLineItemIndex]
                        : null
                    }
                ...
                <div className="bg-white rounded-lg p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="w-full px-4 py-3 bg-white border-2 border-gray-200 rounded-lg min-h-30 max-h-50 overflow-y-auto">
                        {formData.notes ? (
                ...
                <div className="flex items-center justify-between gap-4 pt-6 border-t border-gray-200">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {(!isEditMode ||
                      (isEditMode &&
                        quotationData?.getQuotation?.status === "draft")) && (
                        <button
                          onClick={() => handleSubmit(false)}
                          disabled={isLoading}
                          className="px-5 py-2.5 border-2 border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition-colors disabled:opacity-50 flex items-center font-medium"
                        >
                          <svg
                    ...
                    <button
                      ...
                      disabled={isLoading}
                      className="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center font-medium"
                    >
                      <svg
                        className="w-5 h-5 mr-2"
                        fill="none"
          ...
          <div className="lg:w-[35%] lg:sticky lg:top-6 lg:self-start space-y-4">
            {/* Products & Services */}
            <div className="bg-white rounded-lg p-5 border border-gray-200 shadow-sm hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-blue-900 border-b border-gray-200 pb-2 flex-1">
                  Products & Services
                </h3>
                <div className="text-sm text-blue-600 font-medium">
                  {formData.lineItems.length} item
              ...
              {formData.lineItems.length > 0 && (
                <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-sm text-gray-700">
                    <strong>Tip:</strong> Click any product below to edit its
              ...
              {formData.lineItems.length === 0 ? (
                ...
                <div className="space-y-3 max-h-125 overflow-y-auto">
                  {formData.lineItems.map((item, index) => (
                    <div
                      key={item.id}
                      className={`p-3 bg-white border-2 rounded-lg transition-all group ${editingLineItemIndex === index
                        ...
                        item.selectedOptions.length > 0 && (
                          <div className="mb-3 flex flex-wrap gap-1">
                            {item.selectedOptions.map((opt, optIdx) => (
                              <span
                                key={optIdx}
                                className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-md ${opt.isSubscription
                      ...
                      <div className="flex items-center justify-between gap-3 pt-3 border-t border-gray-200">
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="w-16 px-2 py-1.5 text-sm text-center border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                            disabled={isLoading}
                          />
                        </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">
                      {formData.lineItems.length} item
                      {formData.lineItems.length !== 1 ? "s" : ""} in quotation
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      {appliedCoupons.length > 0 && (
                        <div className="space-y-2 mb-3">
                          {appliedCoupons.map((coupon, idx) => (
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-mono font-bold text-green-800 truncate">
                                  {coupon.coupon.code}
                                </p>
                              ...
                              <button
                                onClick={() =>
                                  handleRemoveCoupon(coupon.coupon.code)
                                }
                      ...
                      {couponsData?.getCoupons &&
                        (() => {
                          ...
                          const availableCoupons =
                            couponsData.getCoupons.filter((c) => {
                              const validFrom = new Date(c.validFrom);
                              const validTo = new Date(c.validTo);
                              const isActive =
                              ...
                              const notApplied = !appliedCoupons.some(
                                (ac) => ac.coupon.id === c.id
                              );
                              return (
                                isActive &&
                                isDateValid &&
                          ...
                          // Group by type
                          const couponsByType = {
                            discount_coupon: availableCoupons.filter(
                              (c) => c.type === "discount_coupon"
                            ),
                            promo_code: availableCoupons.filter(
                              (c) => c.type === "promo_code"
                            ),
                            shipping_coupon: availableCoupons.filter(
                              (c) => c.type === "shipping_coupon"
                            ),
                            additional_discount: availableCoupons.filter(
                              (c) => c.type === "additional_discount"
                            ),
                          };
                          ...
                          return (
                            <div className="space-y-2">
                              ...
                              {Object.entries(couponsByType).map(
                                ([type, coupons]) => {
                                  ...
                                  return (
                                    <div key={type}>
                                      ...
                                      <select
                                        ...
                                        className="w-full px-2 py-1.5 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white disabled:opacity-50 disabled:cursor-not-allowed"
                                      >
                                        <option value="">
                    ...
                    {totalCouponDiscount > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-green-600">
                          Total Discount ({appliedCoupons.length} coupon
                          {appliedCoupons.length !== 1 ? "s" : ""})
                        </span>
                ...
                <div className="px-5 pb-5">
                  <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                    ...
                    <p className="font-semibold text-gray-900">
                      {formData.to.businessName || "Not specified"}
                    </p>
                    <p className="text-sm text-gray-600">
                      {formData.to.email || "No email"}
                    </p>
                  </div>
                </div>
              )}
