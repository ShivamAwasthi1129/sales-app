// components/QuotationTracking.js
...
const STATUS_COLORS = {
  ...
  updated: "bg-orange-100 text-orange-800 border-orange-300",
};
  ...
  return (
    <div className="mt-4 bg-white rounded-lg p-5 border border-gray-200 shadow-sm">
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <span
          className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${STATUS_COLORS[currentStatus]}`}
        >
      ...
      {!isCollapsed && (
        <div
          className="space-y-1 bg-gray-50 rounded-lg p-4"
          style={{
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col items-center shrink-0">
                    <div
                      className={`w-9 h-9 rounded-full flex items-center justify-center ${iconBgColor} ${isLatest ? "ring-4 ring-green-100 animate-pulse" : ""
                        }`}
                    >
                      <svg
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <p
                        className={`font-bold text-sm ${isLatest ? "text-green-600" : "text-gray-800"
                          }`}
                      >
                        {statusLabel}
                      </p>
...
export default function QuotationTracking() {
  ...
  const { data, loading, error, refetch } = useQuery(
    GET_QUOTATIONS_WITH_STATUS_HISTORY,
    {
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl">
        <p className="font-semibold">Error loading quotation tracking</p>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between flex-col sm:flex-row gap-4">
          ...
          <div className="space-y-2 flex-1">
            ...
            <p className="text-base mt-4 text-gray-700">
              Track status and history of all quotations
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div
          className={`bg-blue-50 p-4 rounded-lg shadow-sm border border-blue-100 hover:shadow-md transition-all hover:border-blue-300 cursor-pointer ${filterStatus === "draft" ? "ring-2 ring-blue-500" : ""
            }`}
          onClick={() =>
        ...
        <div
          className={`bg-yellow-50 p-4 rounded-lg shadow-sm border border-yellow-100 hover:shadow-md transition-all hover:border-yellow-300 cursor-pointer ${filterStatus === "sent" ? "ring-2 ring-yellow-500" : ""
            }`}
          onClick={() =>
            setFilterStatus(filterStatus === "sent" ? "all" : "sent")
          }
        >
        ...
        <div
          className={`bg-blue-50 p-4 rounded-lg shadow-sm border border-blue-100 hover:shadow-md transition-all hover:border-blue-300 cursor-pointer ${filterStatus === "accepted" ? "ring-2 ring-blue-500" : ""
            }`}
          onClick={() =>
            setFilterStatus(filterStatus === "accepted" ? "all" : "accepted")
          }
        >
          <p className="text-xs text-blue-700 font-bold uppercase tracking-wide">
            Accepted
        ...
        <div
          className={`bg-red-50 p-4 rounded-lg shadow-sm border border-red-100 hover:shadow-md transition-all hover:border-red-300 cursor-pointer ${filterStatus === "rejected" ? "ring-2 ring-red-500" : ""
            }`}
          onClick={() =>
        ...
        <div
          className={`bg-yellow-50 p-4 rounded-lg shadow-sm border border-yellow-100 hover:shadow-md transition-all hover:border-yellow-300 cursor-pointer ${filterStatus === "expired" ? "ring-2 ring-yellow-500" : ""
            }`}
          onClick={() =>
        ...
        <div
          className={`bg-green-50 p-4 rounded-lg shadow-sm border border-green-100 hover:shadow-md transition-all hover:border-green-300 cursor-pointer ${filterStatus === "paid" ? "ring-2 ring-green-500" : ""
            }`}
          onClick={() =>
            setFilterStatus(filterStatus === "paid" ? "all" : "paid")
          }
        >
        ...
        <div
          className={`bg-orange-50 p-4 rounded-lg shadow-sm border border-orange-100 hover:shadow-md transition-all hover:border-orange-300 cursor-pointer ${filterStatus === "updated" ? "ring-2 ring-orange-500" : ""
            }`}
          onClick={() =>
            setFilterStatus(filterStatus === "updated" ? "all" : "updated")
          }
        >
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-all">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <button
            ...
            className="px-4 py-2 text-gray-700 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors font-medium"
          >
            Clear Filters
          </button>
        </div>
          ...
          <div className="divide-y divide-gray-200">
            {filteredQuotations.map((item) => {
              ...
              return (
                <div
                  key={quotation.id}
                  className={`p-6 hover:bg-gray-50 transition-colors cursor-pointer border-b border-gray-200 last:border-b-0 ${selectedQuotation?.id === quotation.id
                    ? "bg-blue-50 border-l-4 border-blue-500"
                    : ""
                    }`}
                  onClick={() =>
                    setSelectedQuotation(
                      selectedQuotation?.id === quotation.id ? null : item
                    )
                  }
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span
                          className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${STATUS_COLORS[quotation.status]
                            }`}
                        >
                      ...
                      {latestStatus && (
                        <div className="mt-3 text-xs text-gray-500">
                          ...
                          <span className="ml-2">
                            by{" "}
                            {latestStatus.changedByName ||
                              latestStatus.changedBy?.name ||
                              "System"}
                          </span>
                    ...
                    <div className="ml-4">
                      <svg
                        className={`w-5 h-5 text-gray-400 transition-transform ${selectedQuotation?.id === quotation.id
                          ...
                          {item.statusHistory.map((history, index) => (
                            ...
                            >
                              <div className="shrink-0">
                                <div
                                  className={`w-3 h-3 rounded-full mt-1.5 ${STATUS_COLORS[history.status]
                                    }`}
                                ></div>
                              ...
                              <div className="flex-1 pb-4">
                                <div className="flex items-center gap-2 mb-2 flex-wrap">
                                  <span
                                    className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${STATUS_COLORS[history.status]
                                      }`}
                                  >
                                    {STATUS_LABELS[history.status]}
                                  </span>
