    ...
    getSubscriptions: async (_, __, context) => {
      ...
      return subscriptions.map(sub => {
        ...
        return {
          ...
          priceItems: (sub.priceItems || []).filter(price => price != null).map(price => ({
          ...
          configurationSnapshot: (sub.configurationSnapshot || []).map(config => ({
            attributeName: config.attributeName || '',
            optionLabel: config.optionLabel || '',
            priceId: config.priceId?.toString() || null,
