...
// Alias functions for Invoice PDF generation
export const generateInvoicePDF = (invoiceData) => {
  const mappedData = {
    ...invoiceData,
    quotationNo: invoiceData.invoiceNo,
    quotationDate: invoiceData.invoiceDate,
    from: invoiceData.billFrom,
    to: invoiceData.billTo
  };
  return generateQuotationPDF(mappedData);
};

export const downloadInvoicePDF = (invoiceData, filename = null) => {
  const doc = generateInvoicePDF(invoiceData);
  const pdfFilename = filename || `Invoice-${invoiceData.invoiceNo || 'Draft'}.pdf`;
  doc.save(pdfFilename);
};

export const generateInvoicePDFBase64 = (invoiceData) => {
  const mappedData = {
    ...invoiceData,
    quotationNo: invoiceData.invoiceNo,
    quotationDate: invoiceData.invoiceDate,
  ...
  return generateQuotationPDFBase64(mappedData);
};
...
export const generateInvoicePDFBuffer = (invoiceData) => {
  ...
  return generateQuotationPDFBuffer(mappedData);
};
