...
export async function createQuotationPaymentLink(quotationData) {
  ...
  try {
    ...
    if (hasSubscriptions && hasOneTimeItems) {
      ...
      Object.values(subscriptionsByInterval).forEach(group => {
        lineItemsForStripe.push({
          price_data: {
            currency: currency,
            product_data: {
              name: `Quotation ${quotationData.quotationNo || 'Subscription'}`,
              description: `Subscription for Quotation ${quotationData.quotationNo || ''} (${group.intervalCount} ${group.interval}(s))`,
            },
            unit_amount: Math.round(group.total * 100),
            recurring: {
      ...
      // Add one-time items as setup fee (will be charged once with subscription)
      if (oneTimeTotal > 0) {
        lineItemsForStripe.push({
          price_data: {
            currency: currency,
            product_data: {
              name: `Quotation ${quotationData.quotationNo || 'Payment'} - Setup Fee`,
              description: `One-time setup fee for Quotation ${quotationData.quotationNo || ''}`,
            },
      ...
      lineItemsForStripe.push({
        price_data: {
          currency: currency,
          product_data: {
            name: `Quotation ${quotationData.quotationNo || 'Payment'}`,
            description: `Payment for Quotation ${quotationData.quotationNo || ''}`,
          },
          unit_amount: Math.round(quotationData.totalAmount * 100),
        },
        quantity: 1,
      });
    }
