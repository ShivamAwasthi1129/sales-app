    ...
    if (hasSubscriptions && hasOneTimeItems) {
      ...
      Object.values(subscriptionsByInterval).forEach(group => {
        group.items.forEach(item => {
          const itemDescription = item.description 
            ? `${item.description}${item.selectedOptions && item.selectedOptions.length > 0 ? ' - ' + item.selectedOptions.map(opt => `${opt.attributeName}: ${opt.optionLabel}`).join(', ') : ''}`
            : (item.selectedOptions && item.selectedOptions.length > 0 
                ? item.selectedOptions.map(opt => `${opt.attributeName}: ${opt.optionLabel}`).join(', ')
                : `Subscription item from Quotation ${quotationData.quotationNo || ''}`);
      ...
      oneTimeItems.forEach(item => {
        const itemDescription = item.description 
          ...
          : (item.selectedOptions && item.selectedOptions.length > 0 
              ? item.selectedOptions.map(opt => `${opt.attributeName}: ${opt.optionLabel}`).join(', ')
              : `One-time item from Quotation ${quotationData.quotationNo || ''}`);
      ...
      oneTimeItems.forEach(item => {
        const itemDescription = item.description 
          ...
          : (item.selectedOptions && item.selectedOptions.length > 0 
              ? item.selectedOptions.map(opt => `${opt.attributeName}: ${opt.optionLabel}`).join(', ')
              : `Item from Quotation ${quotationData.quotationNo || ''}`);
