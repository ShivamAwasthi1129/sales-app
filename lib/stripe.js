  ...
  try {
    ...
    const sessionParams = {
      ...
      metadata: {
        ...
        couponCode: quotationData.couponCode || '',
        couponDiscount: quotationData.couponDiscount?.toString() || '0',
      },
      success_url: quotationData.successUrl || `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/payment/success?session_id={CHECKOUT_SESSION_ID}`,
    ...
    if (quotationData.couponCode && quotationData.couponDiscount > 0) {
      try {
        // Create a one-time Stripe coupon for this specific checkout
        const discountAmountInCents = Math.round(quotationData.couponDiscount * 100);
        
        const coupon = await stripe.coupons.create({
          amount_off: discountAmountInCents,
          currency: currency,
          duration: 'once',
          name: `Discount: ${quotationData.couponCode}`,
          metadata: {
            quotationNo: quotationData.quotationNo || '',
            originalCouponCode: quotationData.couponCode,
          },
        });
        ...
        console.log(`[Stripe] Created coupon ${coupon.id} for discount ${quotationData.couponDiscount} ${currency.toUpperCase()}`);
      } catch (couponError) {
        console.error('[Stripe] Error creating coupon:', couponError);
